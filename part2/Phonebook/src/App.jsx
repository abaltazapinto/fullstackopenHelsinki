...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    showNotification({ type: 'success', message: `${newName} added to phonebook` });
    ...
    axios.post(baseUrl, newPerson)
      ...
      .catch(error => {
          ...
          showNotification({ type: 'error', message: `Error adding ${newName}`});
      });
  }
  };
  ...
  if (confirmDelete) {
    axios.delete(`${baseUrl}/${id}`)
      .then(response => {
        ...
        showNotification({ type: 'success', message: `${person.name} deleted successfully` });
      })
      .catch(error => console.error('Error deleting person', error));

      showNotification({type: 'error', message:`Information of ${person.name} was already deleted by another user`});
  }
};
   //find persons
